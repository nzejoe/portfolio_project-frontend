...
export const getProducts = createAsyncThunk("product/all", async (payload, { rejectWithValue, getState }) => {
    try {
...
const { actions, reducer } = createSlice({
    ...
    reducers: {
        getFilterProducts: (state, action) => {
            ...
            if (color !== "all") {
                tempProducts = tempProducts.filter((product) => product.variations.colors.includes(color));
            }

            if (brand !== "all") {
                tempProducts = tempProducts.filter((product) => product.variations.brand.includes(brand));
            }
